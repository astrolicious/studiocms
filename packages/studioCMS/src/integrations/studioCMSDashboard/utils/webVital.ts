...
export async function getWebVitals(): Promise<WebVitalsResponseItem[]> {
	...
	if (!AstrojsWebVitals_Metric) {
		logger.warn("@astrojs/web-vitals table not found.  If you have not installed the package you can disregard this warning.")
	...
	try {
		...
		logger.error(`Error getting @astrojs/web-vitals table data.  If you have not installed the package you can disregard this error.\n - ${error}`);
		return [] as WebVitalsResponseItem[];
	}
}
